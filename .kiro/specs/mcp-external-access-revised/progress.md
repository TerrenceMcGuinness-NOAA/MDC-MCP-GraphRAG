...
- **Current auth:** SigV4 only. ~~The JWT authorizer will be *added alongside* SigV4, not replacing it (dual-auth: existing developer path unaffected).~~ **FALSE — CORRECTED 2026-08-06.** AWS documents that an AgentCore Runtime supports **either** IAM SigV4 **or** JWT bearer inbound auth, **not both simultaneously**. Attaching the authorizer would **replace** SigV4 and break the developer path (R7/C6). C8 is resolved as single-mode; see design §11.3a and `decision-log.md`. **Do not deploy the Task 5 authorizer custom resource against the live Runtime.**
...
| 2026-08-06 | C8 | **RESOLVED → PIVOT** | **Single-mode CONFIRMED** by AWS docs: "An AgentCore Runtime can support either IAM SigV4 or JWT Bearer Token based inbound auth, but not both simultaneously." The July inference from the 403 body was correct. Per §11.3a's decision table → execute the Path C Gateway pivot. Runtime stays SigV4-only; Tasks 1–5 carry forward; the Task 5 authorizer custom resource must **never** be applied to the live Runtime. Path C spec: `.kiro/specs/mcp-external-access-alternative-gateway/`. Analysis: `decision-log.md` Parts 1–4. |
| 2026-08-06 | C13 | **NEW** | Tool count/scope split needs re-derivation. Path B's "CI 40 / HPC 48 / developer 51" was computed against the **retired Node** runtime (51 tools). Active Python runtime has **52 tools** (C1), so ≥1 tool is unclassified. Path C R5.6 / Task 5.3a cover this. |
| 2026-08-06 | C14 | **NEW** | Gateway interceptors for HTTP/AgentCore-Runtime targets run in **buffered mode only**, but the server serves FastMCP streamable-http which defaults to SSE (`json_response=False` → `text/event-stream`; verified empirically against pinned `fastmcp==3.2.4`). Path C's trusted-context header channel depends on interceptors firing, so `json_response=True` (or `FASTMCP_JSON_RESPONSE=true`) is required. `stateless_http=True` is orthogonal and must stay. Tracked as DP-7. |
