...
- [x] 0. Phase 46A — AWS EC2 Development Environment Provisioning
  - [x] 0.1 Create `SETUP_AWS/` directory structure and bootstrap entry point
  ...
  - [x] 0.2 Implement numbered provisioning scripts
  ...
  - [x] 0.3 Test provisioning scripts are idempotent
...
- [x] 1. Phase 46A — Foundation: CDK Project and VPC Stack
  - [x] 1.1 Scaffold CDK project and define `MdcVpcStack`
  ...
  - [x] 1.2 Define `MdcSecurityStack` with Secrets Manager and SSM entries
  ...
  - [x] 1.3 Define `MdcDataStack` with Neptune, OpenSearch, EFS, and S3
  ...
  - [x]* 1.4 Write unit tests for CDK stacks
...
- [x] 2. Checkpoint — Verify CDK stacks synthesize cleanly
  - Ensure `cdk synth` succeeds for all three stacks, ask the user if questions arise.

- [x] 3. Phase 46A — Configuration and Secrets Resolution
  - [x] 3.1 Implement `resolveConfig()` in `mcp_server_node/src/config/aws-config.js`
  ...
  - [x]* 3.2 Write property test for configuration caching (Property 12)
  ...
  - [x]* 3.3 Write property test for secret non-exposure (Property 11)
...
- [x] 4. Phase 46B — Database Adapter Interfaces and OpenSearch Adapter
  - [x] 4.1 Define `VectorDatabaseAdapter` and `GraphDatabaseAdapter` interfaces
  ...
  - [x] 4.2 Implement `OpenSearchAdapter` extending `VectorDatabaseAdapter`
  ...
  - [x] 4.3 Implement `ChromaDBLegacyAdapter` wrapping existing `VectorDatabase`
  ...
  - [x]* 4.4 Write property test for adapter output compatibility (Property 2)
  ...
  - [x]* 4.5 Write property test for score normalization (Property 7)
...
- [x] 5. Phase 46B — Neptune Adapter and APOC Transformation
  - [x] 5.1 Implement `NeptuneAdapter` extending `GraphDatabaseAdapter`
  ...
  - [x] 5.2 Implement APOC replacement map and transformation engine
  ...
  - [x] 5.3 Implement `Neo4jLegacyAdapter` wrapping existing `GraphDatabase`
  ...
  - [x]* 5.4 Write property test for APOC transformation semantic preservation (Property 3)
  ...
  - [x]* 5.5 Write unit tests for each APOC replacement
...
- [x] 6. Phase 46B — Backend Selection and UnifiedDataAccess Wiring
  - [x] 6.1 Implement `selectDatabaseBackend()` in `mcp_server_node/src/data/backend-selector.js`
  ...
  - [x] 6.2 Refactor `UnifiedDataAccess` to accept adapter instances
  ...
  - [x]* 6.3 Write property test for tool interface preservation (Property 1)
...
- [x] 7. Checkpoint — Verify adapter layer and backend selection
  - Ensure all tests pass, ask the user if questions arise.

- [x] 8. Phase 46B — ECS Fargate, API Gateway, and CloudFront
  - [x] 8.1 Create Dockerfile for MCP server
  ...
  - [x] 8.2 Implement `MdcServerStack` CDK stack
  ...
  - [x]* 8.3 Write unit tests for `MdcServerStack`
...
- [x] 9. Phase 46B — Health Check and Error Handling
  - [x] 9.1 Implement health check logic in MCP server
  ...
  - [x] 9.2 Implement error handling and resilience patterns
  ...
  - [x]* 9.3 Write property test for health check accuracy (Property 9)
  ...
  - [x]* 9.5 Write property test for retry exponential backoff (Property 13)
...
- [x] 10. Checkpoint — Verify MCP server on ECS with health checks
  - Ensure all tests pass, ask the user if questions arise.

- [x] 11. Phase 46C — OpenSearch Index Creation and Data Migration
  - [x] 11.1 Create OpenSearch index definitions
  ...
  - [x] 11.2 Implement data migration script (`scripts/migrate-to-aws.js`)
  ...
  - [x] 11.3 Implement migration verification
  ...
  - [x]* 11.4 Write property test for data completeness (Property 4)
  ...
  - [x]* 11.5 Write property test for migration idempotence (Property 5)
  ...
  - [x]* 11.6 Write property test for embedding fidelity (Property 6)
...
- [x] 12. Checkpoint — Verify data migration and count parity
  - Ensure all tests pass, ask the user if questions arise.

- [x] 13. Phase 46C — Search Relevance Validation
  - [x] 13.1 Implement search relevance comparison tooling
  ...
  - [x]* 13.2 Write property test for search equivalence (Property 8)
...
- [x] 14. Phase 46D — Ingestion Pipeline Adaptation
  - [x] 14.1 Adapt ingestion scripts for AWS backends
  ...
  - [x]* 14.2 Write integration tests for ingestion pipeline
    - Test full re-ingestion cycle produces expected node/rel/doc counts
    - _Requirements: 15.4_

- [x] 15. Checkpoint — Verify ingestion pipeline on AWS backends
  - Ensure all tests pass, ask the user if questions arise.

- [x] 16. Phase 46E — Validation, Monitoring, and Cutover
  - [x] 16.1 Implement CloudWatch dashboards and alarms
  ...
  - [x] 16.2 Implement MCP client configuration cutover
  ...
  - [x]* 16.3 Write integration tests for full MCP tool suite on AWS
