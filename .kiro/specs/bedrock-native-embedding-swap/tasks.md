...
- [x] 1. Embedding model registry
  - [x] 1.1 Implement `src/data/embedding_registry.py`
  ...
  - [x]* 1.2 Write unit tests for the registry
...
- [x] 2. Embedding provider abstraction
  - [x] 2.1 Scaffold `src/data/embedding_provider.py`
  ...
  - [x] 2.2 Implement `BedrockProvider` request body and response parsing
  ...
  - [x] 2.3 Implement `BedrockProvider` retry/backoff
  ...
  - [x] 2.4 Implement `LocalProvider` import-fail behavior
  ...
  - [x]* 2.5 Unit tests for `BedrockProvider` request body shape
  ...
  - [x]* 2.6 Unit tests for retry schedule and `LocalProvider`
...
- [x] 3. Profile-aware index resolver
  - [x] 3.1 Replace `PRODUCTION_INDICES` with profile-keyed structure
  ...
  - [x] 3.2 Extend `resolve_index` for profile routing
  ...
  - [x]* 3.3 Unit tests for resolver routing
...
- [x] 4. Environment variable + warn line
  - [x] 4.1 Parse and validate `MCP_EMBEDDING_PROFILE` in `src/config/environment.py`
  ...
  - [x] 4.2 Emit one-shot `[WARN]` log line on `mpnet768`
  ...
  - [x]* 4.3 Unit tests for env var parsing
...
- [x] 5. OpenSearchAdapter integration
  - [x] 5.1 Drop the sentence-transformers default and `_mpnet_model` cache
  ...
  - [x] 5.2 Resolve provider in `__init__` and rewrite `_generate_embedding`
  ...
  - [x] 5.3 Pass the active profile through to `resolve_index` in `query`
  ...
  - [x] 5.4 Update `tests/conftest.py` fixtures
  ...
  - [x]* 5.5 Update `tests/unit/test_opensearch_adapter_embedding.py`
...
- [x] 6. Container deps verification
  - [x] 6.1 Verify `pyproject.toml` runtime deps
  ...
  - [x]* 6.2 Document transitive dep audit results
...
- [x] 7. Integration test gated on RUN_INTEGRATION
  - [x]* 7.1 Add `tests/integration/test_bedrock_embedding.py`
...
- [x] 8. Pre-deploy validation
  - [x] 8.1 Run the full pytest suite
  ...
  - [x] 8.2 Checkpoint — Ensure all tests pass, ask the user if questions arise.

- [x] 9. Build, push, deploy
  - [x] 9.1 Build the `python-titan-v1` image
  ...
  - [x] 9.2 Capture image-size delta vs `python-all-tools-v3`
  ...
  - [x] 9.3 Push `python-titan-v1` to ECR
  ...
  - [x] 9.4 `update-agent-runtime` for the staging runtime
  ...
  - [x] 9.5 Poll runtime status until READY
...
- [x] 10. Live validation
  - [x] 10.1 `get_server_info` and `mcp_health_check({deep:true})`
  ...
  - [x] 10.2 `search_documentation({query:"data assimilation cycling"})`
  ...
  - [x] 10.3 `get_knowledge_base_status`
  ...
  - [x] 10.4 Capture verbatim outputs into the phase-report scratch
...
- [x] 11. Documentation
  ...
  - [x] 11.2 Add the `[8.22.3]` CHANGELOG entry
  ...
  - [x] 11.3 Update the steering document
...
- [x] 12. Final checkpoint and commit
  - [x] 12.1 Checkpoint — All tests pass, live validation outputs documented
  ...
  - [x] 12.2 Single commit on `develop_aws` referencing Phase C-2c
