...
- [x]* 1. Write the bug condition exploration test (BEFORE any fix)
...
- [x] 2. Add `tenant_scope` async context manager
...
- [x] 3. Add the `run_tenant_scoped` helper
  ...
  - [x]* 3.1 Unit tests for `tenant_scope` and `run_tenant_scoped`
...
- [x] 4. Thread `catalog` into `register(...)` across tool modules
...
- [x] 5. Wire `semantic_search` tenant-scoped tools
...
- [x] 6. Wire `code_analysis` tenant-scoped tools
...
- [x] 7. Wire `graph_rag` tenant-scoped tools (data tools only)
...
- [x] 8. Wire `operational` tenant-scoped tools
...
- [x] 9. Wire `ee2_compliance` + `workflow_info` tenant-scoped tools
...
- [x]* 10. Write the Fix Checking property/unit test
...
- [x]* 11. Write the Schema-Preservation + Server-Global property tests
...
- [x]* 12. Write the Default-Preservation + Unknown-Tenant tests
...
- [x] 13. Verify the exploration test now passes on fixed code
