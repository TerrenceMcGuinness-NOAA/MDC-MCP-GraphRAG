...
- [ ] 3. Wave 0 — Code change + unit test (the only code change in the spec)

  - [ ] 3.1 Implement the `--prefix` flag in `create-opensearch-indices.js`
  ...
  - [ ] 3.2 Write unit test for the `--prefix` flag
  ...
  - [ ] 3.3 Verify bug condition exploration test now passes (against unit-test mocks)
  ...
  - [ ] 3.5 Git commit: script change + unit test
...
- [ ] 4. Wave 1 — STOP-AND-CONFIRM: Delete broken v17 indices
  ...
  - [ ] 4.1 Confirm the bug exists on the live indices
  ...
  - [ ] 4.2 Delete the three broken indices
...
- [ ] 5. Wave 2 — STOP-AND-CONFIRM: Recreate with correct mapping
  - **⚠️ Requires operator confirmation before proceeding**

  - [ ] 5.1 Run `create-opensearch-indices.js` with `--prefix gw_v17_`
  ...
  - [ ] 5.2 Verify mapping shows `knn_vector` type
...
- [ ] 6. Wave 3 — STOP-AND-CONFIRM: Re-ingest v17 documents
  ...
  - [ ] 6.1 Re-ingest documentation (~28,458 docs, ~2h)
    - ```bash
  ...
  - [ ] 6.2 Re-ingest code (~28,559 docs, ~3h)
    - ```bash
      nohup env PYTHONUNBUFFERED=1 DB_BACKEND=aws OPENSEARCH_ENDPOINT=$OPENSEARCH_ENDPOINT AWS_REGION=us-east-1 MCP_EMBEDDING_PROFILE=titan1024 \
  ...
  - [ ] 6.3 Re-ingest J-Jobs (~92 docs, ~5min)
    - ```bash
      nohup env PYTHONUNBUFFERED=1 DB_BACKEND=aws OPENSEARCH_ENDPOINT=$OPENSEARCH_ENDPOINT AWS_REGION=us-east-1 MCP_EMBEDDING_PROFILE=titan1024 \
  ...
  - [ ] 6.4 Validate document counts match expectations
...
- [ ] 7. Wave 4 — Live validation

  - [ ] 7.1 Verify k-NN search works for v17
  ...
  - [ ] 7.2 Verify documentation search works for v17
  ...
  - [ ] 7.3 Verify knowledge base status
  ...
  - [ ] 7.4 Default-tenant preservation checks
