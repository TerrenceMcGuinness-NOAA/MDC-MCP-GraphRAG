...
- [x] 0. Engagement kickoff (COTS, gated prerequisites)
  - [x] 0.1 Confirm the COTS runtime: `kiro-cli` present + `--no-interactive`
    ...
    - _Tag: research_
  - [x] 0.2 Stand up the COTS-truthful verification path: launch the COTS-local
    ...
    - _Tag: configure_
  - [x] 0.3 Seed `progress.md` (Corrections + Codebase-Patterns from the PoC) and
    start the SDD session for this phase
    - _Requirements: 14.1, 14.2_
    - _Tag: configure_
  - [x] 0.4 Snapshot serving-path module state and record baseline COTS counts
    (`get_knowledge_base_status`, `check_knowledge_integrity`) into `progress.md`
    - _Requirements: 12.4, 13.3_
    - _Tag: configure_

- [x] 1. Manifest schema — add `scope` field
  - [x] 1.1 Extend the manifest model: `scope` is a required common field in
  ...
  - [x] 1.2 Classify all 67 sources in `generate_unified_manifest.py`
  ...
  - [x] 1.3 Bump the manifest version for the schema change; dated note
    - _Requirements: 1.5_
    - _Tag: document_
  - [x]* 1.4 Unit test: schema round-trip; missing `scope` raises; unknown value raises
    - _Requirements: 1.1, 1.2_

- [x] 2. Work_Matrix builder respects `scope`
  - [x] 2.1 Add `scope` to `reingest_stages.yaml` stages; reclassify
  ...
  - [x] 2.2 Verify idempotent migration: a pre-scope `state.json` preserves
  ...
  - [x] 2.3 Regenerate `PROGRESS.md`; confirm the 5→1 `documentation` collapse and
    the 58-unit total for the current catalog
    - _Requirements: 2.2_
  - [x]* 2.4 Unit test: builder yields exactly 55 tenant + 3 shared from a fixture
    catalog + stages
    - _Requirements: 2.1, 2.2_
  - [x]* 2.5 Unit test: migration from a pre-scope state preserves terminal
    statuses; only regenerates non-terminal units
    - _Requirements: 2.3_

- [x] 3. Scope-aware collection namer (feeds Framework Task 2.3)
  - [x] 3.1 Define `resolve_collection_name(source, tenant, version)` in
  ...
  - [x] 3.2 Route `_ingest_common.write_vector_doc`, the four v8 ingesters, and
  ...
  - [x]* 3.3 Unit test: 8 cases across (shared|tenant) × (default|explicit ver) ×
    (empty|non-empty prefix)
    - _Requirements: 3.1, 3.2, 3.4_

- [x] 4. Phase-67 path-rename leak fix
  - [x] 4.1 `workflow_info` — resolve the workflow root from
    ...
    - _Requirements: 4.1_
  - [x] 4.2 `check_knowledge_integrity` coverage-gap check (`_resolve_repo_base`) —
    same tenant-resolved substitution
    - _Requirements: 4.2_
  - [x]* 4.3 Functional assertion: `workflow_info` returns `[OK] pass` in
    `mcp_health_check --functional`
    - _Requirements: 4.3, 4.4_

- [x] 5. ChromaDB adapter — `metadata_sampler`
  - [x] 5.1 Add `sample_metadata(collection, n=20) -> list[dict]` to the ChromaDB
    adapter (ChromaDB `get()` with a limit; `[]` on empty/missing)
    - _Requirements: 5.1_
  - [x] 5.2 Wire the two `check_knowledge_integrity` `[SKIP]` paths (Path
    Consistency, Stale Embeddings) to `sample_metadata`
    - _Requirements: 5.2_
  - [x]* 5.3 Unit test: 3-doc mock returns 3; empty returns `[]`
    - _Requirements: 5.1_

- [x] 6. `get_knowledge_base_status` — fix document count
  - [x] 6.1 Sum `collection.count()` across applicable collections; report the sum
    as `Total Documents`
    - _Requirements: 6.1, 6.3_
  - [x] 6.2 Status `[OK] Healthy` when count > 0 OR the tenant has zero applicable
    collections
    - _Requirements: 6.2_
  - [x]* 6.3 Unit test with a mocked adapter returning 3 collections × N docs
    - _Requirements: 6.1_

- [x] 7. Tenant catalog — document the two-axis model
  - [x] 7.1 Add a worked "adding a non-global-workflow tenant" example in
    `.kiro/steering/11-tenant-roadmap.md` (e.g. `pw_mcp` →
    `supported_repos/parallel-works-mcp`, branch `main`)
    - _Requirements: 7.1_
    - _Tag: document_
  - [x] 7.2 Clarify that `workflow_subdir` is a repo-relative anchor, not a
    global-workflow branch checkout — no `tenants.yaml` schema change
    - _Requirements: 7.2_
    - _Tag: document_

- [x] 8. Framework spec cross-references
  - [x] 8.1 Add Corrections / Codebase-Patterns entries to
    `cots-reingest-ralph-framework/progress.md` for the shared-vs-tenant naming
    rule (unblocks its Task 2.3)
    - _Requirements: 8.1_
    - _Tag: document_
  - [x] 8.2 Add a two-line note to `cots-reingest-ralph-framework/design.md`
    pointing at this phase (scope model) and Requirement 3 (namer)
    - _Requirements: 8.2_
    - _Tag: document_

- [x] 9. EXPDIR — realtime, tenant-derived source reconciliation
  - [x] 9.1 Reconcile the manifest `expdir-configs` source: keep `scope: tenant`,
  ...
  - [x] 9.2 Make `resolve_expdir_base(tenant)` tenant-derived — a per-tenant EXPDIR
  ...
  - [x] 9.3 Confirm `expdir`/`rocoto` `skip` (not fail) for tenants with no
    materialized EXPDIR; keep the write-side `{prefix}Experiment` /
    `{prefix}EXPDIRConfig` labeling unchanged
    - _Requirements: 15.4, 15.5_
  - [x] 9.4 Document the realtime + tenant-localized (gw, gw_v17) nature in steering
    and record the EXPDIR base mapping + Correction in `progress.md`
    - _Requirements: 15.6, 14.1_
    - _Tag: document_
  - [x]* 9.5 Unit test: `resolve_expdir_base` returns per-tenant bases; absent base
    → skip signal; `MCP_EXPDIR_BASE_OVERRIDE` respected
    - _Requirements: 15.3_

- [x] 10. Verification pass (COTS-truthful — via the Task 0.2 method, NOT AWS/gateway)
  - [x] 10.1 `mcp_health_check --deep --detailed --functional` → 11/11 pass, no
    SKIP except optional `community_summaries`; `workflow_info` now `[OK] pass`
    - _Requirements: 10.1, 13.1, 13.2_
    - _Tag: validate_
  - [x] 10.2 `check_knowledge_integrity` → 4/4 checks run (no SKIP for the metadata
    sampler or the coverage-gap path)
    - _Requirements: 10.2, 13.1_
    - _Tag: validate_
  - [x] 10.3 `get_knowledge_base_status` → `Total Documents > 0` (matching the ~15
    live COTS collections), status `[OK]`
    - _Requirements: 10.3, 13.1_
    - _Tag: validate_
  - [x] 10.4 `list_all_sources --include_gaps` → every source has a scope; gap-
    ...
    - _Tag: validate_
  - [x] 10.5 Regression: run the `reingest_state` + ingester unit suites and record
...
- [x] 11. Commit staging (no push)
  - [x] 11.1 Add a dated `[Unreleased]` CHANGELOG entry (scope schema change, the
