...
import { NeptuneAdapter } from './NeptuneAdapter.js';
...
export function selectDatabaseBackend(config = {}) {
  ...
  switch (backend) {
    ...
    case 'aws': {
      ...
      return {
        vectorDB: new OpenSearchAdapter({
          endpoint: config.opensearch?.endpoint || process.env.OPENSEARCH_ENDPOINT || '',
        }),
        graphDB: new NeptuneAdapter({
          endpoint: config.neptune?.endpoint || process.env.NEPTUNE_ENDPOINT || '',
        }),
      };
    }
