...
### Summary
Propagated the `supported_repos/` submodule rename/reorder through the code,
ingestion scripts, and live launch config. The canonical develop checkout was
renamed `supported_repos/global-workflow` → `supported_repos/global-workflow_develop`
(and, with no remaining code references, `rocoto` → `rocoto_NOAA-GSL`,
`global-workflow_dev-v17` → `_dev-gfs.v17`, `global-workflow_gefs-v12` →
`_release-gefs_v12`). The five multi-tenant global-workflow checkouts and their
branches were reconfirmed against `.gitmodules` and `tenants.yaml` (gw=develop,
gw_sfs=dev/sfs, gw_jedi_gfs=dev/jedi-gfs, gw_v17=dev/gfs.v17,
gw_gefs_v12=release/gefs_v12). No re-ingest and no runtime behavior change.

### Key finding
The tenant-aware v8 ingesters need **no** path change: they resolve each tenant's
root as `tenant.workflow_root` = `${MCP_WORKFLOW_MOUNT}/<workflow_subdir>` (logical
subdir names `develop`, `dev-sfs`, `dev-jedi-gfs`, `dev-v17`, `gefs-v12`, unchanged),
and the `.pw_workflow_mount` symlink farm (`setup_pw_workflow_mount.sh`
`SUBDIR_TO_CHECKOUT` table) already bridges those subdirs to the renamed physical
checkouts. The stale references were all in **hardcoded fallback defaults**
(`WORKFLOW_ROOT`/`MCP_WORKFLOW_ROOT`/`GW_REPO` = bare `supported_repos/global-workflow`),
the unified manifest, and deployment launch config.

### Changed (44 files — bare develop path → `_develop`)
- **Active Python server**: `generate_unified_manifest.py` (KNOWN_SOURCES SPOT ×7),
  `src/config/unified_manifest.json` (×7), `run_mcp_stdio.sh` (`MCP_WORKFLOW_ROOT`
  default), `src/tools/workflow_info.py` (`DEFAULT_WORKFLOW_ROOT`),
  `src/tools/semantic_search.py` (`_resolve_repo_base` default + docstring),
  `src/tools/smoke_queries.py`, `smoke_test_tools.py`, `test_workflow_info_tools.py`.
- **Legacy Node-server Python ingesters** (`mcp_server_node/scripts/`): all
  `WORKFLOW_ROOT`/`EE2_ROOT`-adjacent defaults across ingest_code_v8/_v7,
  ingest_shell_graph_v8, ingest_jjobs_v8, ingest_fortran_graph, ingest_config_files,
  ingest_env_variables, ingest_cross_language_bridges, ingest_python_graph,
  ingest_jinja2_templates, ingest_code_graph_enriched_v6, ingest_ci_test_cases,
  create_shell_fortran_bridge, ingest_external_lib_stubs, documentation_sources_config,
  plus parse-ver-files.js, run-track-b-ingestion.sh, INGESTION_SCRIPT_GUIDE.md.
- **Node-server runtime (js)**: EE2VectorStore, OperationalTools, WorkflowInfoTools,
  SemanticSearchTools, SubmoduleGraphIngester, CMakeGraphIngester, GitHubGraphIngester,
  __tests__/setup.js.
- **Deployment env / launch config**: `.github/mcp.json`, `SETUP/mcp-env.sh`,
  `SETUP_AWS/mcp-env-aws.sh`, `SETUP/docker-mcp/catalogs/eib-local.yaml`,
  `docker-compose.mcp-standalone.yaml`, `SETUP/provisioning/{common,00-users,
  11-docker-mcp-gateway,16-lmod,12-static-mode-gateway}.sh`.

### Deliberately NOT changed (old name is correct there)
- **EFS access-point root** paths (`${STAGING_MNT}/supported_repos/global-workflow`,
  `<EFS>/…`): `scripts/overnight_v17_ingest*.sh`, `remediate_v17_reingest.sh`,
  `_populate_worktrees.py` docstring, `populate_workflow_efs*.sh`. Worktrees are
  created under this fixed EFS path by `workflow_subdir`; it is independent of the
  local submodule directory names.
- **CDK infra** (`infrastructure/cdk/**`, `SETUP_AWS/provisioning/cdk/**`): the EFS
  access-point `path: '/supported_repos/global-workflow'` is an AWS resource
  definition, not the local checkout.
- **Ingested data snapshot** `mcp_server_node/data/community_contexts.json` (67 stale
  path strings) — regenerated by re-ingestion, not hand-edited.
- **Historical records**: `docs/**`, `sdd_framework/workflows/**`,
  `mcp_architecture/**`, prior CHANGELOG entries, `.kiro/specs|steering/**` — left as
  point-in-time records.

### Notes
- The regex used a negative lookahead (`supported_repos/global-workflow(?![-.\w])`)
  so it never touched `global-workflow_develop`, the other tenant suffixes,
  `global-workflow.wiki`, or the `global-workflow-docs-v8-0-0` collection target.
- `tenants.yaml` itself required no edit (its `workflow_subdir` values are the logical
  names, unchanged). The `setup_pw_workflow_mount.sh` SPOT table and
  `branch_ground_truth.py`/`validate_code_awareness.py` were already updated to the
  new physical checkout names prior to this change.
